...
function SettingRow({ title, description, children, className = '' }: { title: string; description: string; children: React.ReactNode; className?: string }) {
  return <div className={`flex items-center justify-between gap-4 py-2 ${className}`}><div className="flex min-w-0 flex-1 flex-col gap-0.5"><strong>{title}</strong><span>{description}</span></div>{children}</div>;
}
