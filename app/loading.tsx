...
export default function Loading() {
  return (
    <div className="flex min-w-0 flex-col gap-4 pb-6" role="status" aria-label="Loading page">
      ...
      <div className="overflow-hidden rounded-2xl border border-border/60 bg-card px-5 pb-5 pt-3 sm:px-6 sm:pb-6 sm:pt-4"><div className="flex items-center gap-4"><Skeleton className="size-16 rounded-2xl" /><div className="space-y-2"><Skeleton className="h-5 w-44" /><Skeleton className="h-3 w-64" /></div></div><div className="mt-5 grid grid-cols-2 gap-3 border-t border-border/50 pt-5 sm:grid-cols-4">{[0, 1, 2, 3].map((item) => <div key={item} className="flex items-center gap-2.5"><Skeleton className="size-8 rounded-xl" /><div className="space-y-2"><Skeleton className="h-4 w-12" /><Skeleton className="h-2.5 w-20" /></div></div>)}</div></div>
